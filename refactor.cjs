...
const path = 'd:\\DEV\\grimoire\\src\\components\\VTTToolbar.svelte';
let content = fs.readFileSync(path, 'utf8');

// Add activeMenu state and component refs
if (content.includes("let ambientTextInput = $state('');")) {
...
const html_replacement = `<svelte:window onkeydown={handleKeydown} onclick={() => activeMenu = null} />
...
const pattern = /<svelte:window onkeydown=\{handleKeydown\} \/>.*?<!-- Modals -->/s;
content = content.replace(pattern, html_replacement + '\n\n<!-- Modals -->');
...
// REPLACE CAREFULLY: only from .vtt-toolbar to just before /* Sélecteur de carte */ or .picker-backdrop
...
/*
  ...
  .picker-backdrop {
*/
// We want to replace from `.vtt-toolbar {` up to just before `/* Sélecteur de carte */` OR `.campaign-title-input`
const css_pattern = /\.vtt-toolbar \{.*?(?=\.campaign-title-input \{)/s;
content = content.replace(css_pattern, css_replacement + '\n\n');

// Also remove .overflow-grid and tools-overflow-panel
content = content.replace(/\/\* ── Overflow panel ──────────────────────────────────────────── \*\/.*?<\/style>/s, '</style>');
