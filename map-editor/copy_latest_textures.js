...
const targetPath = 'd:\\DEV\\grimoire\\map-editor\\inkarnate_textures.json';
...
function getLatestJson() {
  ...
  const files = fs.readdirSync(downloadsDir)
    .filter(f => f.startsWith('inkarnate_textures') && f.endsWith('.json'))
    .map(f => {
...
if (latest) {
  ...
  console.error("❌ Aucun fichier inkarnate_textures*.json trouvé dans les Téléchargements !");
  process.exit(1);
}
